...
#' Interactive plot for HDLV.
#' @param x An HDLV object.
#' @param col Color for the level set.
#' @param r0 Visualization size for each cluster.
#' @param l_size Line width for the connection between clusters.
Intplot.HDLV = function(x, col=NULL, r0=1, l_size=20){
  manipulate(fn_plot(x=x,max(x$density)*lv, r0=r0, l_size=l_size, col=col), 
             #lv=slider(x$lv.min,max(x$density)))
             lv = slider(0.1,1,label="Density Level"))
}
fn_plot= function(x=x, lv, r0=r0, l_size= l_size, col=col){
  M_sig = x$modes[x$sig.clu,]
  M_mds = cmdscale(dist(M_sig))
  row.names(M_mds) = x$sig.clu
  ...
  minM_dist = min(dist(M_mds))  
  ## if exist input level sequence
  C_info = CluInfo(x$data, x$density, x$MS.label, lv_seq=c(lv,lv), h=x$h, lv_min=x$lv.min)
  N_size = C_info$Size/S_max
  conn_list = C_info$Conn
  
  ##
  if(is.null(col)){
    col5 = colorRampPalette(c("lightgreen","limegreen","seagreen","orange","red","red2","red3","red4","black"))(1001)
    lv_min = x$lv.min
    lv_max = max(x$density)
    lv_r = round((lv-lv_min)/(lv_max-lv_min)*1000+1)
    col = col5[lv_r]
  }
  
  plot(M_mds, xlim=c(-1,1), ylim=c(-1,1), xlab="",ylab="", axes=F, frame.plot=T, col="white")
  for(i_mode in 1:length(x$sig.clu)){
    draw.circle(x= M_mds[i_mode,1], y= M_mds[i_mode,2], radius = N_size[1,i_mode]* minM_dist/2*r0, border=NA, col= col)
  }
  for(i_tmp in 1:(length(x$sig.clu)-1)){
    for(j_tmp in (i_tmp+1):length(x$sig.clu)){
      if(conn_list[[1]][i_tmp, j_tmp]==1){
        lines(x=c(M_mds[i_tmp,1], M_mds[j_tmp,1]), y= c(M_mds[i_tmp,2], M_mds[j_tmp,2]), lwd=l_size*(N_size[1,i_tmp]+ N_size[1,j_tmp]), col=col)
        
      }
    }
  }
  m_pch = rep(13, length(x$sig.clu))
  m_pch[which(N_size[1,]==0)] = 1
  points(M_mds, cex=2, pch=m_pch)
  legend("topright", c("Modes (> Level)", "Modes (< Level)"), pch=c(13,1), pt.cex=2)
}
